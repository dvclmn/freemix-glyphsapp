...
class AlignmentPalette(PalettePlugin):
    ...
    #     return displayStrFromNumbers(
    #         globalCenterX, globalCenterXmax
    #     ), displayStrFromNumbers(globalCenterY, globalCenterYmax)
    ...
    #                     if (
    #                         zone.size > 0
    #                         and node2.y >= zone.position
    #                         and node2.y <= zone.position + zone.size
    #                     ):
    ...
    def settings(self):
        ...
        self.paletteView = Window(
            (width, height), minSize=(width, height - 10), maxSize=(width, height + 200)
        )
        self.paletteView.group = Group((0, 0, width, height))

        posy = self.marginTop
        # set up fields for center
        headlineBbox = NSAttributedString.alloc().initWithString_attributes_(
            "Bounding butts",
            {NSFontAttributeName: NSFont.boldSystemFontOfSize_(smallSize)},
        )
        self.paletteView.group.headlineBbox = TextBox(
            (10, posy, innerWidth, 18), headlineBbox, sizeStyle="small"
        )
        posy += self.lineSpacing
        self.paletteView.group.centerXLabel = TextBox(
            (10, posy + 3, innerWidth, 18), "center x", sizeStyle="small"
        )
        # self.posy_centerX = posy
        # self.paletteView.group.centerX = ArrowEditText(
        #     (self.posx_TextField, posy, textFieldWidth, textFieldHeight),
        ...
        # )
        posy += self.lineSpacing
        self.paletteView.group.centerYLabel = TextBox(
            (10, posy + 3, innerWidth, 18), "center y", sizeStyle="small"
        )
        # self.posy_centerY = posy
        # self.paletteView.group.centerY = ArrowEditText(
        #     (self.posx_TextField, posy, textFieldWidth, textFieldHeight),
        #     callback=self.editTextCallback,
        #     continuous=False,
        #     readOnly=False,
        ...
        headlineOvershoot = NSAttributedString.alloc().initWithString_attributes_(
            "Overshoot",
            {
                NSFontAttributeName: NSFont.boldSystemFontOfSize_(
                    NSFont.systemFontSizeForControlSize_(smallSize)
                )
            },
        )
        self.paletteView.group.headlineOvershoot = TextBox(
            (10, posy, innerWidth, 18), headlineOvershoot, sizeStyle="small"
        )
        posy += self.lineSpacing
        self.paletteView.group, "lineAbove", HorizontalLine(
            (self.marginLeft, posy - 3, innerWidth, 1)
        )
        for i in range(MAX_ZONES):
            setattr(
                self.paletteView.group,
                "name" + str(i),
                TextBox((10, posy, innerWidth, 18), "", sizeStyle="small"),
            )
            setattr(
                self.paletteView.group,
                "value" + str(i),
                TextBox(
                    (self.posx_TextField, posy, textFieldWidth - 3, textFieldHeight),
                    "",
                    sizeStyle="small",
                    alignment="right",
                ),
            ...
            setattr(
                self.paletteView.group,
                "line" + str(i),
                HorizontalLine((self.marginLeft, posy - 3, innerWidth, 1)),
            )
    ...
    def update(self, sender=None):
        ...
        if (
            not self.font.selectedLayers
            or len(self.font.selectedLayers) > MAX_GLYPHS_COUNT
        ):
        ...
        if self.font.selectedLayers:
            print("Turned off center calculation")
            pass
            # determine centers
            ...
            # self.paletteView.group.centerY.show(False)
            pass
            print("Turned off center display")
        ...
        if self.font.selectedLayers:
            ...
            pass
            print("Turned off overshoot calculation")
        ...
        for i in range(MAX_ZONES):
            pass
            print("Turned off overshoot display")
            # try:
    ...
    #         pass
    #         print("Turned off center setting")
